// Enhanced AI Cybersecurity Platform Frontend - FIXED VERSION
// Bug fixes: Chat history, localStorage backup, responsive mobile, crash prevention
...
let pollTimeout = null; // For preventing memory leaks
...
function setupEventListeners() {
    ...
    chatInput.addEventListener('input', () => {
        ...
        chatInput.style.height = Math.min(chatInput.scrollHeight, 200) + 'px'; // Max height 200px
    });
    ...
    // Handle page visibility change (prevent crash when switching tabs/apps)
    document.addEventListener('visibilitychange', () => {
        if (!document.hidden) {
            // Page became visible again - restore if needed
...
function handleLogout() {
    ...
    localStorage.removeItem('chatMessages'); // Clear chat backup
...
async function initializeDashboard() {
    ...
    await loadChatHistory(); // BUG FIX: Load chat history from server
    checkAIHealth();
}
...
// BUG FIX: New function to load chat history from server
async function loadChatHistory() {
    ...
    try {
        ...
        if (data.success && data.messages && data.messages.length > 0) {
            ...
            // If no server history, try localStorage backup
            loadChatFromLocalStorage();
        }
    } catch (error) {
...
// BUG FIX: Save chat to localStorage for backup
function saveChatToLocalStorage() {
    try {
        const messagesContainer = document.getElementById('chat-messages');
        const messages = [];

        messagesContainer.querySelectorAll('.message:not(.thinking-message)').forEach(msgDiv => {
        ...
        localStorage.setItem('chatMessagesSession', currentSessionId);
    } catch (error) {
        console.error('Failed to save chat to localStorage:', error);
    }
}

// BUG FIX: Load chat from localStorage
function loadChatFromLocalStorage() {
    try {
        const savedMessages = localStorage.getItem('chatMessages');
        const savedSessionId = localStorage.getItem('chatMessagesSession');

        // Only load if it's the same session
        if (savedMessages && savedSessionId === currentSessionId) {
...
async function pollScanProgressInChat(scanId, thinkingMsg, generateReport) {
    // BUG FIX: Clear previous timeout to prevent memory leak
    if (pollTimeout) {
        clearTimeout(pollTimeout);
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            if (data.completed) {
                ...
                // Continue polling - BUG FIX: Store timeout ID
                pollTimeout = setTimeout(() => pollScanProgressInChat(scanId, thinkingMsg, generateReport), 2000);
            }
        }
    } catch (error) {
        removeThinkingAnimation(thinkingMsg);
...
// BUG FIX: Updated addMessageToUI with localStorage save
function addMessageToUI(role, content, shouldSave = true) {
    const messagesContainer = document.getElementById('chat-messages');
    ...
    // BUG FIX: Save to localStorage after adding message
    if (shouldSave) {
